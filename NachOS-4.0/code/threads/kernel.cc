...
void Kernel::ReadString2KeyBoard(int toAddr, char *buffer, int size)
{
    ...
    do
    {
        buffer[i] = kernel->synchConsoleIn->GetChar(); // read char from console
        oneChar = (int)buffer[i];
        kernel->machine->WriteMem(toAddr + i, 1, oneChar); // Copy buffer from System memory space to User memory space

        i++;
    } while (i < size && oneChar != 0 && buffer[i - 1] != '\n');
...
int Kernel::CreateFile()
{
    int addr = kernel->machine->ReadRegister(4);
    char *sysBuff = User2System(addr, MAX_LENGTH);
    if(strlen(sysBuff)==0)
        return -1;
    if(!sysBuff)
        return -1;
    bool code = kernel->fileSystem->create(sysBuff);
    return code ? 0 : -1;
}

int Kernel::RemoveFile()
{
    int addr = kernel->machine->ReadRegister(4);
    char *sysBuff = User2System(addr, MAX_LENGTH);
    if(strlen(sysBuff)==0)
    ...
    if(!sysBuff)
        return -1;
    bool code = kernel->fileSystem->delete(sysBuff);
    return code ? 0 : -1;
}